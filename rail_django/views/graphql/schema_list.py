"""
Schema list view for displaying available GraphQL schemas.

This module provides SchemaListView, which lists all registered GraphQL schemas
and their metadata in either JSON or HTML format.
"""
...
from .utils import _get_authenticated_user, _host_allowed
...
class SchemaListView(View):
    ...
    Supports both JSON and HTML output formats. HTML format renders a
    dashboard-style view with schema statistics.

    Attributes:
        template_name: Template used for HTML rendering

    Usage:
        # In urls.py
        from rail_django.views.graphql import SchemaListView
    ...
    def get(self, request: HttpRequest) -> JsonResponse:
        ...
        1. The 'format' query parameter ('json' or 'html')
        2. The Accept header (prefers text/html if present)

        Args:
            request: HTTP request object

        Returns:
            JsonResponse with schema list or rendered HTML template
        """
        ...
        try:
            ...
            schemas = self._serialize_schemas(
                schema_registry.list_schemas(), request=request
            )
    ...
    def _serialize_schemas(
        ...
        """
        Serialize schema objects to dictionaries.

        Filters out schemas that the current user cannot access (e.g., GraphiQL
        schemas with host restrictions).

        Args:
            schema_list: List of schema info objects from the registry
            request: Optional HTTP request for access checking
    ...
    def _is_graphiql_visible(
        self, request: HttpRequest, schema_info: dict[str, Any]
    ) -> bool:
        """
        Check if a GraphiQL schema should be visible to the current request.
        ...
        Returns:
            True if the schema should be visible
        """
    ...
    def _build_context(self, schemas: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        Args:
            schemas: List of serialized schema dictionaries

        Returns:
            Context dictionary with schemas and statistics
        """
        total = len(schemas)
    ...
    def _wants_html(self, request: HttpRequest) -> bool:
        """
        Determine if the client prefers HTML output.

        Args:
            request: HTTP request object

        Returns:
            True if HTML output is preferred
        """
