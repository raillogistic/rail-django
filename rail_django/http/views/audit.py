...
from rail_django.security.events.types import EventType, Severity
    ...
    def get(self, request: HttpRequest) -> JsonResponse:
        ...
        try:
            ...
            if hasattr(audit_logger, "get_security_report"):
                report = audit_logger.get_security_report(hours=hours) or {}
            ...
            report = {
                "period_hours": report.get("period_hours", hours),
                "failed_logins": report.get("failed_logins", 0),
                "successful_logins": report.get("successful_logins", 0),
                "suspicious_activities": report.get("suspicious_activities", 0),
                "rate_limited_requests": report.get("rate_limited_requests", 0),
                "total_events": report.get("total_events", queryset.count()),
...
class AuditEventTypesView(View):
    ...
    def get(self, request: HttpRequest) -> JsonResponse:
        ...
        for legacy in ("low", "medium", "high"):
            if legacy not in severities:
                severities.append(legacy)
        return JsonResponse(
            {
