...
from ..form.fixtures.canonical_model import CANONICAL_PRODUCT_FIXTURE
...
@pytest.fixture
def schema_harness():
    return build_schema(schema_name="test_detail_v2_canonical_crud", apps=["test_app"])
...
def admin_user():
    ...
    return User.objects.create_superuser(
        username="detail_v2_admin",
        email="detail_v2_admin@example.com",
        password="pass12345",
    )
...
def gql_client(schema_harness, admin_user):
    return RailGraphQLTestClient(
        ...
        schema_name="test_detail_v2_canonical_crud",
        user=admin_user,
    )


def test_detail_contract_query_resolves_for_authorized_user(gql_client):
    query = """
    ...
    assert result["contract"]["queryRoot"] == "product"
    assert result["contract"]["identifierArg"] == "id"
...
def test_pages_query_and_authorization_path(gql_client, limited_user):
    ...
    assert (
        allowed["data"][CANONICAL_PRODUCT_FIXTURE.page_query]["pageInfo"]["totalCount"]
        >= 1
    )
...
def test_create_update_delete_canonical_mutation_paths(gql_client, limited_user):
    ...
    denied_create_errors = denied_create["data"][
        CANONICAL_PRODUCT_FIXTURE.create_operation
    ]["errors"]
